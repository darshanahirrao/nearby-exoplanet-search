...
def bin_series(t, y, e, step=10 / 1440):
    ...
    sw = np.bincount(ix, weights=w)
    valid = sw > 0
    return (
        np.bincount(ix, weights=t * w)[valid] / sw[valid],
        np.bincount(ix, weights=y * w)[valid] / sw[valid],
        1 / np.sqrt(sw[valid]),
    )
...
def load_target(tic, flux_column="PDCSAP_FLUX", window_days=1.5, inject=None):
    ...
    for path in sorted((ROOT / "data/lightcurves" / str(tic)).glob("*_lc.fits")):
        with fits.open(path, memmap=False) as h:
            ...
            y = np.array(d[flux_column], dtype=float)
            e = np.array(d[flux_column + "_ERR"], dtype=float)
            ...
            if inject is not None:
                phase = (t - inject["epoch"] + 0.5 * inject["period"]) % inject[
                    "period"
                ] - 0.5 * inject["period"]
            ...
            splits = np.r_[0, np.where(np.diff(t) > 0.3)[0] + 1, len(t)]
            for left, right in zip(splits[:-1], splits[1:]):
                tt, yy, ee = t[left:right], y[left:right], e[left:right]
                if len(tt) < 80 or np.ptp(tt) < 0.8:
                    continue
                tt, yy, ee = bin_series(tt, yy, ee)
                if len(tt) < 30:
                    continue
                cadence = np.median(np.diff(tt))
                window = max(9, int(window_days / cadence) // 2 * 2 + 1)
                ...
                keep = (tt > tt[0] + 0.10) & (tt < tt[-1] - 0.10) & (flat < 1 + 6 * sigma)
...
def split_campaigns(df):
    df = df.copy()
    df["campaign"] = np.r_[0, np.cumsum(np.diff(df.time) > 100)]
    sizes = df.groupby("campaign").size()
    best = int(sizes.idxmax())
    if len(sizes) > 1:
        discovery = df[df.campaign == best].copy()
        validation = df[df.campaign != best].copy()
        ...
        if len(sectors) >= 4:
            ...
            method = "no independent sector holdout available"
...
def scan(df, low, high, star, max_signals=4):
    ...
    high = min(float(high), baseline * 0.48, 100)
    if high <= low or len(t) < 300:
        return [], {}
    # A logarithmic grid bounds transit phase drift by about 1/4 of the shortest searched duration.
    durations = np.array([0.025, 0.04, 0.06, 0.09, 0.13, 0.18, 0.25])
    durations = durations[durations < low * 0.12]
    ...
    remaining = np.ones(len(t), dtype=bool)
    signals = []
    for iteration in range(max_signals):
        if remaining.sum() < 300:
            break
        ...
        epoch = float(power.transit_time[k])
        depth = float(power.depth[k])
        snr = float(power.depth_snr[k])
        ...
        ref = model.power(
            fine,
            np.unique(
                np.clip(duration * np.array([0.75, 1, 1.25]), durations.min(), durations.max())
            ),
            objective="likelihood",
            oversample=10,
        )
        j = int(np.nanargmax(ref.power))
        period = float(ref.period[j])
        duration = float(ref.duration[j])
        epoch = float(ref.transit_time[j])
        depth = float(ref.depth[j])
        snr = float(ref.depth_snr[j])
        mad = robust_sigma(power.power)
        sde = float((power.power[k] - np.median(power.power)) / max(mad, 1e-9))
        sig = {
            "period_days": period,
            "epoch_btjd": epoch,
        ...
        sig["radius_earth_estimate"] = float(np.sqrt(max(depth, 0)) * float(star.Rad) / 0.0091577)
        ...
        sig["central_circular_duration_days"] = float(
            central_duration(period, star, sig["radius_earth_estimate"])
        )
        signals.append(sig)
        if snr < 5:
            break
...
def event_checks(df, period, epoch, duration):
    ...
    for cycle in np.unique(cycles[transit]):
        ...
        outside = (np.abs(t - center) > duration) & (np.abs(t - center) < max(4 * duration, 0.3))
        if inside.sum() < 3 or outside.sum() < 8:
            continue
        w = 1 / e[inside] ** 2
        wo = 1 / e[outside] ** 2
    ...
    if records:
        dep = np.array([x["depth"] for x in records])
        err = np.array([x["error"] for x in records])
        w = 1 / err**2
        total = float(np.sum(w * dep) / np.sqrt(sum(w)))
        positive = int(sum(dep > 0))
        sum_sq = sum(max(x["snr"], 0) ** 2 for x in records)
        ...
        for parity in [0, 1]:
            ...
            if sel.any():
                sides.append(
                    (float(sum(w[sel] * dep[sel]) / sum(w[sel])), float(1 / np.sqrt(sum(w[sel]))))
                )
        odd_even = (
            abs(sides[0][0] - sides[1][0]) / np.hypot(sides[0][1], sides[1][1])
            if len(sides) == 2
            else None
        )
    else:
        ...
        odd_even = None
    return {
        "n_observed_events": len(records),
        "n_positive_events": positive,
        "event_snr": records,
        "fixed_ephemeris_snr": total,
        "single_event_power_fraction": dominance,
...
def known_matches(tic, period):
    found = []
    for fname, idcol, namecol, pcol in [
        ("exofop_toi.csv", "TIC ID", "TOI", "Period (days)"),
        ("exofop_ctoi.csv", "TIC ID", "CTOI", "Period (days)"),
    ]:
        p = ROOT / "data/catalogs" / fname
        if not p.exists():
            continue
        data = pd.read_csv(p)
        data = data[pd.to_numeric(data[idcol], errors="coerce") == tic]
        for _, row in data.iterrows():
            kp = pd.to_numeric(row[pcol], errors="coerce")
            if np.isfinite(kp) and kp > 0:
                for ratio in [0.5, 1, 2, 3, 1 / 3]:
                    if abs(period / (kp * ratio) - 1) < 0.01:
                        found.append(
                            {
                                "catalogue": fname,
                                "id": str(row[namecol]),
    ...
    if p.exists():
        data = pd.read_csv(p)
        ids = pd.to_numeric(
            data.tic_id.astype(str).str.replace("TIC ", "", regex=False), errors="coerce"
        )
            ...
            if np.isfinite(kp) and kp > 0:
                for ratio in [0.5, 1, 2, 3, 1 / 3]:
                    if abs(period / (kp * ratio) - 1) < 0.01:
                        found.append(
                            {
                                "catalogue": "confirmed_planets.csv",
                                "id": row.pl_name,
                                "period": float(kp),
                                "period_ratio": ratio,
                            }
...
def analyze(tic, max_signals=4, window_days=1.5, output_tag=""):
    ...
    outdir = RESULT / (str(tic) + output_tag)
    outdir.mkdir(exist_ok=True)
    result = {
        "tic": int(tic),
        "started_utc": datetime.now(timezone.utc).isoformat(),
        "status": "running",
        "claims": "Exploratory signals only. No confirmed discovery.",
    }
    try:
        stars = pd.read_csv(ROOT / "data/catalogs/all_hz_targets.csv")
        star = stars[stars.TIC == int(tic)].iloc[0]
        df, meta = load_target(tic, window_days=window_days)
        hz = stellar_hz(star)
        df, masks = mask_known_inner(df, int(tic), max(1, 0.85 * hz["inner_period"]))
        discovery, validation, split = split_campaigns(df)
        result.update(
            star=star.to_dict(),
            files=meta,
            points=len(df),
            discovery_points=len(discovery),
            validation_points=len(validation),
            split_method=split,
            discovery_sectors=sorted(map(int, discovery.sector.unique())),
            validation_sectors=sorted(map(int, validation.sector.unique())),
            detrend_window_days=window_days,
            hz_physics=hz,
            known_inner_transit_masks=masks,
        ...
        (outdir / "frozen_discovery.json").write_text(
            json.dumps(
                {
                    "signals": sig,
                    "config": config,
        ...
        for s in sig:
            s["validation"] = event_checks(
                validation, s["period_days"], s["epoch_btjd"], s["duration_days"]
            )
            s["known_matches"] = known_matches(int(tic), s["period_days"])
            flags = []
            if s["known_matches"]:
                flags.append("known_catalogue_signal_or_harmonic")
            if s["n_observed_events"] < 3:
                flags.append("fewer_than_three_discovery_events")
            if (
                s.get("single_event_power_fraction") is not None
                and s["single_event_power_fraction"] > 0.6
            ):
                flags.append("dominated_by_one_event")
            if s.get("odd_even_sigma") is not None and s["odd_even_sigma"] > 3:
                flags.append("odd_even_discrepancy")
            if not s["in_optimistic_hz"]:
                flags.append("outside_nominal_optimistic_hz")
            if s["duration_days"] > 1.6 * s["central_circular_duration_days"]:
                flags.append("long_relative_to_circular_transit")
            if not 0.5 <= s["radius_earth_estimate"] <= 2:
                flags.append("outside_initial_small_planet_radius_range")
            if len(validation) == 0:
                flags.append("no_independent_holdout")
            elif s["validation"]["n_observed_events"] < 2:
                flags.append("insufficient_holdout_events")
            elif (s["validation"]["fixed_ephemeris_snr"] or 0) < 5:
                flags.append("not_recovered_at_fixed_ephemeris")
            if s["nominal_bls_snr"] < 7:
                flags.append("low_nominal_snr")
            s["screening_flags"] = flags
            s["screening_status"] = (
                "requires_detailed_vetting" if not flags else "flagged_screening_signal"
            )
...
def main():
    ...
    p.add_argument("--tics", nargs="*", type=int)
    p.add_argument("--all-downloaded", action="store_true")
    ...
    if a.all_downloaded:
        for p in sorted((ROOT / "data/download_status").glob("*.json")):
            ...
            if (
                s["state"] == "downloaded"
                and len(s["files"]) >= 3
                and not (RESULT / (str(s["tic"]) + a.tag) / "result.json").exists()
            ):
                tics.append(s["tic"])
    for tic in tics:
        ...
        print(
            ...
            [
                (round(s["period_days"], 5), round(s["nominal_bls_snr"], 1), s["screening_flags"])
