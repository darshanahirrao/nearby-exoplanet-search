...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        default=1800,
        help="maximum seconds waiting for target downloads",
    )
    a = p.parse_args()
    targets = pd.read_csv(a.target_list).TIC.astype(int).tolist()
    print(
        "BEGIN",
        ...
        while pending or futures:
            for tic in list(pending):
                ...
                try:
                    ...
                    continue  # a download may be atomically replaced on next loop
                ...
                if len(status.get("files", [])) < 2:
                    skipped.append(
                        {
                            "tic": tic,
                            "reason": "fewer than two downloaded sectors",
            ...
            for f in completed:
                ...
                try:
                    ...
                    print(
                        ...
                        len(signals),
                        "unflagged",
                        sum(not s["screening_flags"] for s in signals),
                        r.get("error", ""),
                        flush=True,
                    )
                except Exception as exc:
                    skipped.append(dict(tic=tic, reason=repr(exc)))
                    print(tic, "worker_error", repr(exc), flush=True)
            if time.monotonic() > deadline and pending and not futures:
                skipped.extend(
                    {"tic": tic, "reason": "download unavailable before wait limit"}
                    for tic in pending
                )
                pending.clear()
            state = {
                "updated_utc": datetime.now(timezone.utc).isoformat(),
                "target_list": str(Path(a.target_list).resolve().relative_to(ROOT)),
                "completed": done,
                "running": list(futures.values()),
