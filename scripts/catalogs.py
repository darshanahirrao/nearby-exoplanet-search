...
def fetch(item):
    ...
    return {"file": str(p.relative_to(ROOT)), "url": url,
...
def main():
    ...
    sources = [("ReadMe", base+"ReadMe")]+[(f"table{i}.dat",base+f"table{i}.dat") for i in [3,5,6]]
    sources += [("exofop_toi.csv", "https://exofop.ipac.caltech.edu/tess/download_toi.php?sort=toi&output=csv"),
                ("exofop_ctoi.csv", "https://exofop.ipac.caltech.edu/tess/download_ctoi.php?sort=ctoi&output=csv")]
    ...
    sources.append(("confirmed_planets.csv", "https://exoplanetarchive.ipac.caltech.edu/TAP/sync?"+requests.compat.urlencode({"query":query,"format":"csv"})))
    manifest=[]
    with ThreadPoolExecutor(max_workers=3) as pool:
        jobs={pool.submit(fetch,s):s for s in sources}
        for j,s in jobs.items():
            try:
                v=j.result();manifest.append(v);print(s[0], v['bytes'],flush=True)
            except Exception as e:
                manifest.append({"file":s[0],"url":s[1],"error":repr(e)});print(s[0],repr(e),flush=True)
    (CAT/"manifest.json").write_text(json.dumps(manifest,indent=2))
    tables=[]
    for num in [3,5]:
        t=ascii.read(CAT/f"table{num}.dat",readme=str(CAT/"ReadMe"),format="cds").to_pandas()
        t['catalogue_table']=num; tables.append(t)
    ...
    try:
        ...
        stars['known_ctoi_host']=stars.TIC.isin(pd.to_numeric(ct[ctcol],errors='coerce').dropna().astype('int64'))
    except Exception:
        stars['known_ctoi_host']=False
    # Broad feasible pool; observed photometric noise will replace this crude ranking.
    eligible=stars[(stars.Rad.between(.1,.65)) & (stars.Mass>0) &
                   stars.Teff.between(2600,4500) & (stars.Dist<=100) &
                   (stars.Tmag<=14) & stars.earth_period_days.between(3,90)].copy()
    eligible['priority_score']=eligible.earth_depth_ppm*10**(-0.2*(eligible.Tmag-10))*np.sqrt(30/eligible.earth_period_days)
    ...
    print('all',len(stars),'eligible',len(eligible),'without_TOI',sum(~eligible.known_toi_host),flush=True)
    print(eligible[['TIC','Tmag','Rad','Teff','Dist','earth_period_days','earth_depth_ppm','known_toi_host','known_ctoi_host']].head(30).to_string(index=False),flush=True)
