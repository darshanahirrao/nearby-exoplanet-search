...
def main():
    ...
    a = p.parse_args()
    name = f"campaign_{a.offset}_{a.count}"
    logs = ROOT / "logs"
    logs.mkdir(exist_ok=True)
    targets = pd.read_csv(ROOT / "data/catalogs/ranked_targets.csv")
    targets = targets[~targets.known_toi_host & ~targets.known_ctoi_host].iloc[
        a.offset : a.offset + a.count
    ]
    targetfile = ROOT / f"target_batch_{a.offset}_{a.count}.csv"
    targets.to_csv(targetfile, index=False)
    state = {
    ...
    def checkpoint(stage):
        ...
        (logs / f"{name}.json").write_text(json.dumps(state, indent=2))
    ...
    try:
        ...
        with (
            (logs / f"{name}_download.log").open("w") as dl,
            (logs / f"{name}_search.log").open("w") as sl,
        ):
            ...
            try:
                while downloader.poll() is None or searcher.poll() is None:
                    time.sleep(10)
                if downloader.returncode or searcher.returncode:
                    raise RuntimeError(
                        f"download exit={downloader.returncode}; search exit={searcher.returncode}"
                    )
            finally:
            ...
            for tic in targets.TIC.astype(int):
                ...
                for s in json.loads(result.read_text()).get("signals", []):
                    if not s["screening_flags"]:
                        subprocess.run(
                            command(
                                "vet.py", "--tic", tic, "--signal", s["seed_iteration"], "--refined"
                            ),
                            cwd=ROOT,
                            stdout=output,
                            stderr=subprocess.STDOUT,
