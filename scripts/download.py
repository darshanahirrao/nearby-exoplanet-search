...
DATA = ROOT / "data"
for p in ["products", "lightcurves", "download_status"]:
    (DATA / p).mkdir(exist_ok=True, parents=True)


def download_target(tic, max_sectors=18):
    tic = int(tic)
    start = time.monotonic()
    status = {"tic": tic, "files": [], "errors": []}
    dest = DATA / "download_status" / f"{tic}.json"
    try:
        cache = DATA / "products" / f"{tic}.ecsv"
        if cache.exists():
            table = Table.read(cache)
        else:
        ...
        for row in table:
            ...
            url = "https://mast.stsci.edu/api/v0.1/Download/file?" + requests.compat.urlencode(
                {"uri": uri}
            )
            if not p.exists():
                for attempt in range(3):
                    try:
                        r = session.get(url, timeout=(15, 100))
                        r.raise_for_status()
                        if not r.content.startswith(b"SIMPLE"):
                            raise ValueError("response is not a FITS primary HDU")
                        ...
                        if attempt == 2:
                            status["errors"].append({"file": filename, "error": repr(e)})
                        else:
        ...
        status.update(state="error", error=repr(e), traceback=traceback.format_exc())
...
def main():
    ...
    p.add_argument("--max-sectors", type=int, default=18)
    ...
    if a.tics:
        ...
        t = t.iloc[a.offset : a.offset + a.count]
        t.to_csv(ROOT / f"target_batch_{a.offset}_{a.count}.csv", index=False)
